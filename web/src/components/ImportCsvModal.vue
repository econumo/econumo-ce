        ...
        >
          <q-card-section>
            <div class="settings-toolbar-mobile">
              <div>
                <h4 class="settings-toolbar-mobile-title">
                  {{ $t('modals.import_csv.header') }}
                </h4>
              </div>
            </div>
            ...
            <!-- Field Mapping -->
            <div
              v-if="csvFile && csvColumns.length > 0"
              class="import-csv-mapping"
            >
              ...
              <div class="import-csv-account-row">
                ...
                <q-btn
                  ...
                  dense
                  :icon="
                    accountMode === 'csv_column' ? 'swap_horiz' : 'description'
                  "
                  color="primary"
                  @click="
                    handleAccountModeChange(
                      accountMode === 'csv_column'
                        ? 'existing_account'
                        : 'csv_column'
                    )
                  "
                ...
                >
                  <q-tooltip>{{
                    $t(
                      accountMode === 'csv_column'
                        ? 'modals.import_csv.switch_to_manual'
                        : 'modals.import_csv.switch_to_csv'
                    )
                  }}</q-tooltip>
                </q-btn>
              </div>
              ...
              <div class="import-csv-field-row">
                ...
                <q-btn
                  ...
                  dense
                  :icon="
                    dateMode === 'csv_column' ? 'swap_horiz' : 'description'
                  "
                  color="primary"
                  @click="
                    handleDateModeChange(
                      dateMode === 'csv_column' ? 'manual' : 'csv_column'
                    )
                  "
              ...
              <div v-if="amountMode === 'single'" class="import-csv-amount-row">
                ...
                <q-btn
                  ...
                  @click="
                    amountMode = 'dual';
                    handleAmountModeChange('dual');
                  "
                  class="import-csv-amount-switch"
                >
                  <q-tooltip>{{
                    $t('modals.import_csv.amount_mode.switch_to_dual')
                  }}</q-tooltip>
                </q-btn>
              </div>
              ...
              <div v-if="amountMode === 'dual'" class="import-csv-dual-amounts">
                <div class="import-csv-amount-row">
                  <div class="responsive-modal-control import-csv-amount-field">
                    <q-select
                      ...
                      :options="csvColumnOptionsWithNone"
                      :label="
                        $t('modals.import_csv.fields.amount_inflow') + ' *'
                      "
                      outlined
                      emit-value
                      map-options
                  ...
                  <q-btn
                    ...
                    @click="
                      amountMode = 'single';
                      handleAmountModeChange('single');
                    "
                    class="import-csv-amount-switch"
                  >
                    <q-tooltip>{{
                      $t('modals.import_csv.amount_mode.switch_to_single')
                    }}</q-tooltip>
                  </q-btn>
                </div>
                ...
                <div class="responsive-modal-control import-csv-outflow-field">
                  <q-select
                    ...
                    :options="csvColumnOptionsWithNone"
                    :label="
                      $t('modals.import_csv.fields.amount_outflow') + ' *'
                    "
                    outlined
                    emit-value
                    map-options
              ...
              <div class="import-csv-field-row">
                ...
                <q-btn
                  ...
                  dense
                  :icon="
                    categoryMode === 'csv_column' ? 'swap_horiz' : 'description'
                  "
                  color="primary"
                  @click="
                    handleCategoryModeChange(
                      categoryMode === 'csv_column'
                        ? 'existing_entity'
                        : 'csv_column'
                    )
                  "
              ...
              <div class="import-csv-field-row">
                <div class="responsive-modal-control import-csv-field-control">
                  ...
                  <q-input
                    ...
                    :label="$t('modals.import_csv.fields.description')"
                    :placeholder="
                      $t('modals.import_csv.fields.description_placeholder')
                    "
                    outlined
                    class="form-input full-width"
                  />
                ...
                <q-btn
                  ...
                  dense
                  :icon="
                    descriptionMode === 'csv_column'
                      ? 'swap_horiz'
                      : 'description'
                  "
                  color="primary"
                  @click="
                    handleDescriptionModeChange(
                      descriptionMode === 'csv_column' ? 'manual' : 'csv_column'
                    )
                  "
                  class="import-csv-field-switch"
                >
              ...
              <div class="import-csv-field-row">
                ...
                <q-btn
                  ...
                  dense
                  :icon="
                    payeeMode === 'csv_column' ? 'swap_horiz' : 'description'
                  "
                  color="primary"
                  @click="
                    handlePayeeModeChange(
                      payeeMode === 'csv_column'
                        ? 'existing_entity'
                        : 'csv_column'
                    )
                  "
              ...
              <div class="import-csv-field-row">
                ...
                <q-btn
                  ...
                  dense
                  :icon="
                    tagMode === 'csv_column' ? 'swap_horiz' : 'description'
                  "
                  color="primary"
                  @click="
                    handleTagModeChange(
          ...
          <q-card-section
            v-if="isImporting && isChunkedImport"
            class="import-csv-progress-section"
          >
            <q-linear-progress
...
const isShowingResult = ref(false); // Flag to prevent cancel emit when showing result modal
...
const existingAccountOptions = computed(() => {
  ...
  return accountsStore.accountsOrdered
    .filter((account) => {
      ...
      const userAccess = account.sharedAccess.find(
...
function handleDialogHide() {
  ...
  // Don't emit cancel if we're transitioning to show the result modal
...
function autoDetectFields(headers: string[]) {
  ...
  const inflowIndex = lowerHeaders.findIndex((h) =>
    h.includes(amountInflowLabel)
  );
  const outflowIndex = lowerHeaders.findIndex((h) =>
    h.includes(amountOutflowLabel)
  );
...
function autoDetectSingleField(
  field: 'account' | 'date' | 'category' | 'description' | 'payee' | 'tag'
) {
  ...
  if (field === 'account') {
    ...
    if (accountIndex >= 0)
      fieldMapping.value.account = csvColumns.value[accountIndex];
  } else if (field === 'date') {
...
): Array<{ message: string; rows: number[] }> {
...
async function uploadChunk(
  ...
  imported: number;
  skipped: number;
  errors: Array<{ message: string; rows: number[] }>;
}> {
  ...
  const file = new File([blob], `chunk_${chunkIndex}.csv`, {
    type: 'text/csv',
  });
  ...
  try {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      try {
        ...
        if (
          accountMode.value === 'existing_account' &&
          fieldMapping.value.account
        ) {
        ...
        if (
          categoryMode.value === 'existing_entity' &&
          fieldMapping.value.category
        ) {
          uploadOptions.categoryId = fieldMapping.value.category as Id;
        }
        if (
          descriptionMode.value === 'manual' &&
        ...
        const errorMessage = `Chunk ${i + 1} failed: ${
          chunkError?.data?.message || chunkError?.message || 'Unknown error'
        }`;
    ...
    const allErrors = Array.from(errorMap.entries()).map(
      ([message, rowSet]) => ({
        message,
